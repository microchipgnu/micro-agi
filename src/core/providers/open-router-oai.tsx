...
export type ValidChatModel = any;
...
const openAiClientContext = AI.createContext<() => OpenAIClient>(
  _.once(() => {
    ...
    const config = {
      ...
      fetch: ((globalThis as any)._nextOriginalFetch ?? globalThis.fetch).bind(
        globalThis
      ),
    };
...
function estimateFunctionTokenCount(
...
function tokenLimitForChatModel(
  ...
  const functionEstimate =
    chatModelSupportsFunctions(model) && functionDefinitions
      ? estimateFunctionTokenCount(functionDefinitions)
      : 0;
...
function coalesceToolCallMessages(
  ...
  return messages.reduce<OpenAIClient.Chat.ChatCompletionMessageParam[]>(
 ...
 */
export async function* OpenRouterChatModel(
  ...
  if (
    props.forcedFunction &&
    !Object.keys(props.functionDefinitions).includes(props.forcedFunction)
  ) {
    throw new AIJSXError(
  ...
  let promptTokenLimit = tokenLimitForChatModel(
    props.model,
    props.functionDefinitions
  );
  ...
  const unmatchedFunctionCallIds =
    getUnmatchedFunctionIds(conversationMessages);

  const messages = await Promise.all(
    conversationMessages.map(
      async (
        message
      ): Promise<OpenAIClient.Chat.ChatCompletionMessageParam> => {
        switch (message.type) {
          case "system":
            return {
              role: "system",
          ...
          case "functionCall":
            if (
              message.element.props.id &&
              !unmatchedFunctionCallIds.has(message.element.props.id)
            ) {
  ...
  const openaiTools = !props.functionDefinitions
    ...
    : Object.entries(
        props.functionDefinitions
      ).map<OpenAIClient.Chat.ChatCompletionTool>(
        ([functionName, functionDefinition]) => ({
          function: {
            name: functionName,
            description: functionDefinition.description,
            parameters: functionDefinition.parameters as Record<
              string,
              unknown
            >,
          },
  ...
  const chatCompletionRequest: OpenAIClient.Chat.Completions.ChatCompletionCreateParamsStreaming =
    {
      ...
      tool_choice: props.forcedFunction
        ? {
            function: { name: props.forcedFunction },
            type: "function" as const,
          }
        : undefined,
      stop: props.stop,
      logit_bias: props.logitBias
        ? logitBiasOfTokens(props.logitBias)
        : undefined,
        ...
        (async function* () {
          ...
          while (delta != null) {
            ...
            if (partialArgs !== undefined) {
              yield (
                <FunctionCall id={id} partial name={name} args={partialArgs} />
              );
            }
...
const _patchedUntruncateJson =
  "default" in untruncateJson ? untruncateJson.default : untruncateJson;

function patchedUntruncateJson(str: string): string {
